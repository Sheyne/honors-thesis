...
as formally described in Figure \ref{subtype-definitions}, 
defines literals as subtypes of there respective general types.
For example \(\LitT(\text{``Hello"})\subtype\StringT\). It defines
record subtypes structurally, that is all the supertype's fields 
are present in the subtype and the types these fields in the subtype
...
are subtypes of other unions as long as all of the types in the 
...
\begin{align*}
    ...
    \UnionT(\Type_{1,1}, ...)&\subtype\UnionT(\Type_{2,1}, ...) 
    &&\textif \forall T\in \{\Type_{1,i}\} \exists T' \in \{\Type_{2,i}\} \suchthat T\subtype T' \\
    \InterT(\Type_{1,1}, ...)&\subtype\InterT(\Type_{2,1}, ...) 
    &&\textif \forall T\in \{\Type_{1,i}\} \forall T' \in \{\Type_{2,i}\} T\subtype T' \\
    \InterT(..., \Type_i, ...)&\subtype\ObjT_1 &&\textif \Type_i\subtype\ObjT_1  \\
